...
# Set search strings for successful connections and timeout length
WEB_STRING="Your web app is running"
SQL_STRING="succeeded"
TIMEOUT="15"

# Output to console
echo "Run the following on the Azure VMs:"
echo "-------------------------------------------"
...
((curl --connect-timeout $TIMEOUT -s https://$WEBAPP2_HOSTNAME | grep '$WEB_STRING' > /dev/null) && echo 'https://$WEBAPP2_HOSTNAME Success!' || echo 'https://$WEBAPP2_HOSTNAME Failed!') && \
(curl --connect-timeout $TIMEOUT -s $WEBAPP2_IP | grep -q '$WEB_STRING' && echo $WEBAPP2_IP' Success!' || echo $WEBAPP2_IP' Failed!')" > test.sh